...
signal toggle_external_oxidizing(external_inventory_owner)
signal update_oxidizing_menu(inventory_data_input, inventory_data_output, type, craft_quantity)
signal update_progress_bar(progress)
signal set_external_inventory_owner()
signal check_for_new_owner()

@export var inventory_data_input: InventoryDataOxidizable
...
func _on_hurtbox_player_interact(user):
		...
		toggle_external_oxidizing.emit(self)
		update_oxidizing_menu.emit(inventory_data_input, inventory_data_output, type, capacity)
		update_progress_bar.emit(progress)
		is_start_button_disabled()

func reupdate_oxidizing_menu():
	update_oxidizing_menu.emit(inventory_data_input, inventory_data_output, type, capacity)

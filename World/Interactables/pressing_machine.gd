...
signal toggle_external_pressing(external_inventory_owner)
signal update_pressing_menu(inventory_data_input, inventory_data_output, type, craft_quantity)
signal update_progress_bar(progress)
signal set_external_inventory_owner()
signal check_for_new_owner()

@export var inventory_data_input: InventoryDataPressable
...
func _on_hurtbox_player_interact(user):
		...
		toggle_external_pressing.emit(self)
		update_pressing_menu.emit(inventory_data_input, inventory_data_output, type, capacity)
		update_progress_bar.emit(progress)
		is_start_button_disabled()

func reupdate_pressing_menu():
	update_pressing_menu.emit(inventory_data_input, inventory_data_output, type, capacity)

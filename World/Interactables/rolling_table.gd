...
signal toggle_external_rolling(external_inventory_owner)
signal update_rolling_menu(inventory_data_input, inventory_data_output, type, craft_quantity)
signal update_progress_bar(progress)
signal set_external_inventory_owner()
signal check_for_new_owner()

@export var inventory_data_input: InventoryDataRollable
@export var inventory_data_output: InventoryDataOxidizable
...
func _on_hurtbox_player_interact(user):
		...
		toggle_external_rolling.emit(self)
		update_rolling_menu.emit(inventory_data_input, inventory_data_output, type, capacity)
		update_progress_bar.emit(progress)
		is_start_button_disabled()

func reupdate_rolling_menu():
	update_rolling_menu.emit(inventory_data_input, inventory_data_output, type, capacity)

...
func connect_signals():
	...
	for node in get_tree().get_nodes_in_group("external_oxidizing"):
		node.update_oxidizing_menu.connect(update_oxidizing_menu)
...
func update_oxidizing_menu(_inventory_data_input, _inventory_data_output, type, _craft_quantity):
...
func check_for_new_owner():
	external_inv_owner.reupdate_oxidizing_menu()
